        ...
        <div>
            ...
            <div class="container">
                <div class='p-1 mb-0 bg-dark text-light' style='opacity:90%;'>
                    ...
                    <button type=submit name="id" class="btn btn-primary btn-sm">Nº do Pedido</button>
                    <button type=submit name="valor_max" class="btn btn-primary btn-sm">Valor</button>
                    <button type=submit name="data" class="btn btn-primary btn-sm">Data</button>
                    <button type=submit value="CA" name="status_pedido" class="btn btn-primary btn-sm">Apenas Cancelados</button>
                    <button type=submit value="CO" name="status_pedido" class="btn btn-primary btn-sm">Apenas Confirmados</button>
                    <button type=submit value="EE" name="status_pedido" class="btn btn-primary btn-sm">Apenas Em Espera</button>
                    <button type=submit value="OK" name="status_pedido" class="btn btn-primary btn-sm">Apenas Aceitos</button>
                </div>
                </form>
                <?php
                if(isset($_POST['valor_max'])){
                ...
                echo "<div>";
                    ...
                    echo "</div>";
                    echo "<table class='table table-bordered table-light' style='opacity:90%;'>";
                        echo "<tr>";
                            echo "<th>NºPedido</th>";
                            echo "<th>Cliente</th>";
                        ...
                        while($pedido=mysqli_fetch_array($sql)){
                            echo "<tr>";
                                ...
                                echo "<td><a href='detalhescompra.php?idpedido=$idpedido'><button type='submit' class='btn btn-primary btn-sm'>Ver Detalhes</button></a>  <a href='apagarpedido.php?idpedido=$idpedido'><button type='submit' class='btn btn-primary btn-sm'>Apagar Pedido</button></a></td>";
                            echo "</tr>";
                        }
                    echo "</table>";
                        ...
                        //exibe a paginação
                            echo "<table>";
                                for($i = 1; $i <  $numpaginas + 1; $i++){
                                    ...
                                    if(isset($_POST['valor_max'])){
                                        echo "<button type=submit name='valor_max' class='btn btn-primary'>$i</button>";
                                    }
                                    if(isset($_POST['data'])){
                                        echo "<button type=submit name='data' class='btn btn-primary'>$i</button>";
                                    }
                                    if(isset($_POST['status_pedido'])){
                                        $status_pedido=$_POST['status_pedido'];
                                        echo "<button type=submit name='status_pedido' value='$status_pedido' class='btn btn-primary'>$i</button>";
                                    }
                                    if((!isset($_POST['valor_max']) and !isset($_POST['data']) and !isset($_POST['status_pedido'])) or isset($_POST['id'])){
                                        echo "<button type=submit name='id' class='btn btn-primary'>$i</button>";
                                    }
                                    echo "</form></td>";
                                }
                            echo "</table>";
